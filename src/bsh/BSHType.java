...
implements BshClassManager.Listener
{
  ...
  private SimpleNode getTypeNode() {
  ...
  public String getTypeDescriptor(CallStack callstack, Interpreter interpreter, 
                                  String defaultPackage, int incDims) {
    String desc = getTypeDescriptor(callstack, interpreter, defaultPackage);
    for (int i = 0; i < incDims; i++) {
      desc = "[" + desc;
    }
    return desc;
  }
  ...
  public Class getType( CallStack callstack, Interpreter interpreter, int incDims) throws EvalError {
    Class clas = getType(callstack, interpreter); // must call this first
    if ( incDims > 0 ) {
      try {
        int[] dims = new int[incDims + getArrayDims()];
        Object obj = Array.newInstance(getBaseType(), dims);
        clas = obj.getClass(); 
      } catch(Exception e) {
        throw new EvalError("Couldn't construct array type", 
                            this, callstack );
  ...
  /**
    Clears the cached type value.  It is necessary to call this when
    evaluating a declaration such as "int a, b[];" where the two variables 
